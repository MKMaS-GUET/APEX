...
void DictionaryBuilder::SaveDict(uint max_threads) {
    ...
    std::ofstream predicate_out = std::ofstream(dict_path_ + "/predicates", std::ofstream::out | std::ofstream::binary);
    std::ofstream subject_out =
        std::ofstream(dict_path_ + "/subjects/nodes", std::ofstream::out | std::ofstream::binary);
    std::ofstream object_out = std::ofstream(dict_path_ + "/objects/nodes", std::ofstream::out | std::ofstream::binary);
    std::ofstream shared_out = std::ofstream(dict_path_ + "/shared/nodes", std::ofstream::out | std::ofstream::binary);
...
void DictionaryBuilder::Build() {
    ...
    std::cout << "save dictionary takes " << std::chrono::duration<double, std::milli>(end - beg).count() << " ms."
              << std::endl;
...
void DictionaryBuilder::EncodeRDF(hash_map<uint, std::vector<std::pair<uint, uint>>>& pso) {
    ...
    while (fin >> s >> p) {
        ...
        it = objects_.find(o);
        oid = (it != objects_.end()) ? shared_.size() + subjects_.size() + it->second : shared_.find(o)->second;
