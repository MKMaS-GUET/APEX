...
const char *IndexRetriever::ID2String(uint id, SPARQLParser::Term::Position pos) { return dict_.ID2String(id, pos); }

uint IndexRetriever::Term2ID(const SPARQLParser::Term &term) { return dict_.String2ID(term.value, term.position); }

// ?s p ?o
std::span<uint> IndexRetriever::GetSSet(uint pid) { return predicate_index_.GetSSet(pid); }

// ?s p ?o
std::span<uint> IndexRetriever::GetOSet(uint pid) { return predicate_index_.GetOSet(pid); }
...
uint IndexRetriever::GetSSetSize(uint pid) { return predicate_index_.GetSSetSize(pid); }

uint IndexRetriever::GetOSetSize(uint pid) { return predicate_index_.GetOSetSize(pid); }
...
uint IndexRetriever::GetBySPSize(uint sid, uint pid) { return GetBySP(sid, pid).size(); }

uint IndexRetriever::GetByOPSize(uint oid, uint pid) { return GetByOP(oid, pid).size(); }

uint IndexRetriever::GetBySOSize(uint sid, uint oid) { return GetBySO(sid, oid).size(); }

uint IndexRetriever::predicate_cnt() { return dict_.predicate_cnt(); }

uint IndexRetriever::shared_cnt() { return dict_.shared_cnt(); }