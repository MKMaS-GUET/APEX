...
const char* IndexRetriever::ID2String(uint id, SPARQLParser::Term::Position pos) {
...
// ?s p ?o
std::span<uint> IndexRetriever::GetPreSSet(uint pid) {
    return predicate_index_.GetSSet(pid);
}

// ?s p ?o
std::span<uint> IndexRetriever::GetPreOSet(uint pid) {
