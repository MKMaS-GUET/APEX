...
void CsDaaMap::Build(std::pair<std::vector<uint>&, std::vector<ulong>&> spo_map,
    ...
    ulong file_size = (shared_id_size_ * (spo_width + ops_width) + not_shared_size * not_shared_width + 7ul) / 8ul;
        ...
        for (uint i = 0; i < spo_width + ops_width; i++) {
            ...
            if (i < spo_width) {
                ...
                if (idx < cs_id_width_.second)
                    ...
                    bit =
                        ops_map.second[id - 1] & (1ull << (daa_offset_width_.second - 1 - (idx - cs_id_width_.second)));
            }
...
uint CsDaaMap::DAAOffsetOf(uint id, Permutation permutation) {
    ...
    if (id <= shared_id_size_) {
        ...
        if (permutation == Permutation::kOPS) {
            bit_start =
                (ulong(id) - 1) * shared_width_ + (cs_id_width_.first + daa_offset_width_.first) + cs_id_width_.second;
