...
void Endpoint::query(const httplib::Request& req, httplib::Response& res) {
    ...
    if (db_name != "" && db_index != 0) {
        ...
        // db_index->shared_cnt()); if (!query_plan->zero_result())
        //     executor->Query();
        ...
        //             writer.StartArray();
        //             writer.String(db_index->ID2String(*it, SPARQLParser::Term::Position::kPredicate));
        //             writer.EndArray();
        //         }
        //     } else {
...
bool Endpoint::start_server(const std::string& ip, const std::string& port, const std::string& db) {
    ...
    std::string base_url = "/avpjoin";

    db_index = std::make_shared<IndexRetriever>(db);
    db_name = db;

    svr.Get(base_url + "/sparql",
            [this](const httplib::Request& req, httplib::Response& res) { this->query(req, res); });  // query on RDF
    svr.Post(base_url + "/sparql",
             [this](const httplib::Request& req, httplib::Response& res) { this->query(req, res); });  // query on RDF
    svr.Options(base_url + "/sparql", [](const httplib::Request& req, httplib::Response& res) { res.status = 200; });
